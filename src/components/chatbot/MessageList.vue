<template>
  <div class="sc-message-list" ref="scrollList" :style="{backgroundColor: colors.messageList.bg}">
    <Message v-for="(message, idx) in messages" :message="message" :chatImageUrl="chatImageUrl(message.author)" :authorName="authorName(message.author)" :key="idx" :colors="colors" :messageStyling="messageStyling" :isUser="isUser(message.author)" />
...
export default {
  ...
  methods: {
    ...
    chatImageUrl (author) {
      return this.profile(author).name == 'chatbot' ? '/static/img/chatbot.png' : this.profile(author).imageUrl
    },
    authorName (author) {
      return this.profile(author).name
    },
    isUser (author) {
      return this.profile(author).type === 'user'
    }
  },
  computed: {
    defaultChatIcon () {
